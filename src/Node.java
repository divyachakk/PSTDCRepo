...
public class Node<T> {
	...
	ArrayList<Node> children = new ArrayList<Node>(); //create a new ArrayList of Nodes, children, to be filled one by one with the addNode function and newNodes
	...
	boolean addNode(Node node) { //boolean method to addNodes
		...
		} else if (amIASuffix(node) || (tokenSequence.size() == 0)) { //however if the added node is a suffix and the size of the tokenSequence equals zero

			int i = 0;
			while (i < children.size() && !found) { //while i is parsing through the children ArrayList and not found
				found = children.get(i).addNode(node); //set found variable equal to children.get(i).addNode(node), checking to see if the node already exists
	...
	boolean amIASuffix(Node node) { //checking to see if the added node is a suffix of the input
		...
		if (tokenSequence.equals(node.getTokenSequence().subList(node.getTokenSequence().size() - tokenSequence.size(),
				node.getTokenSequence().size()))) { //check if the particular node's value equals a suffix value at the end of the input string
	...
	void print(int numSpacesBefore) { //print method with overloaded constructor
		...
		for (int i = 1; i <= numSpacesBefore; i++) { //going from one through the number of spaces filled in the constructor
		...
		for (int j = 0; j < children.size(); j++) { // for each node in children
			children.get(j).print(numSpacesBefore + 1); //print out the respective node value with overloaded constructor
		}
